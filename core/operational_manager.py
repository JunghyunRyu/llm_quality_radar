...
class OperationalManager:
    ...
    async def get_quality_report(self, test_id: str = None, days: int = 30) -> Dict[str, Any]:
        """품질 보고서 생성"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
    ...
    async def _create_error_notification(self, test_id: str, error_message: str):
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO notifications (type, title, message, test_id)
                VALUES (?, ?, ?, ?)
            ''', ("error", "테스트 실패", f"테스트 {test_id} 실행 중 오류 발생: {error_message}", test_id))
    ...
    async def get_notifications(self, unread_only: bool = False) -> List[Dict[str, Any]]:
        """알림 조회"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
