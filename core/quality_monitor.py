...
class QualityMonitor:
    ...
    async def _assess_performance(self, mcp_client) -> float:
        ...
        try:
            ...
            self.performance_data = {"metrics": performance_metrics, "scores": scores}
    ...
    async def _assess_accessibility(self, mcp_client) -> float:
        ...
        try:
            ...
            # 스크린 리더 호환성 평가
            screen_reader_score = self._evaluate_screen_reader_compatibility(
    ...
    async def _assess_seo(self, mcp_client) -> float:
        ...
        try:
            ...
            # 종합 점수 계산
            overall_score = (
                meta_score + heading_score + alt_text_score + internal_links_score
            ) / 4
    ...
    async def _assess_functionality(self, mcp_client) -> float:
        ...
        try:
            ...
            # 종합 점수 계산
            overall_score = (
                broken_links_score + js_errors_score + form_validation_score
            ) / 3
        ...
        try:
            ...
            if checks.get("aria_labels"):
                for aria_check in checks["aria_labels"]:
                    total_checks += 1
                    if aria_check.get("aria_label") or aria_check.get(
                        "aria_labelledby"
                    ):
                        passed_checks += 1
            ...
            for form in forms:
                ...
                if inputs:
                    valid_inputs = sum(
                        ...
                        if input_field.get("validation", True)
                    )
