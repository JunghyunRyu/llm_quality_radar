...
async def log_requests(request: Request, call_next):
    ...
    logger.info(
        "REQ %s %s %s from %s",
        request_id, request.method, request.url.path, request.client.host if request.client else "-",
    )
    try:
        response = await call_next(request)
        logger.info(
            "RES %s %s %s -> %s",
            request_id, request.method, request.url.path, getattr(response, 'status_code', '-')
        )
        return response
    except Exception:
                ...
                if contents and isinstance(contents, (list, tuple)):
                    for item in contents:
                        ...
                        if isinstance(text_value, str) and 'data:image' in text_value:
                            import re
                            m = re.search(r'(data:image/[^;]+;base64,[A-Za-z0-9+/=]+)', text_value)
                            if m:
                                return {
                                    "status": "captured",
                                    "description": "페이지 스크린샷",
...
def extract_screenshot_info(event_str: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # 스크린샷 관련 키워드 확장
        screenshot_keywords = ["screenshot", "take_screenshot", "capture", "image", "스크린샷", "캡처"]
        has_screenshot = any(keyword in event_str.lower() for keyword in screenshot_keywords)
        ...
        if has_screenshot:
            ...
            for i, pattern in enumerate(patterns):
                ...
                if match:
                    if 'data:image' in match.group(0):
                        ...
                        print(f"✅ 패턴 {i+1}로 완전한 data URL 추출 성공: {len(match.group(0))}자")
                    else:
                        # base64 데이터만 있는 경우
                        data = match.group(1) if match.groups() else match.group(0)
                        if len(data) > 100:
                            screenshot_info["image_data"] = f"data:image/png;base64,{data}"
                            print(f"✅ 패턴 {i+1}로 base64 데이터 추출 성공: {len(data)}자")
                    break
            ...
            if "image_data" not in screenshot_info and "file_path" not in screenshot_info:
                ...
                screenshot_info["description"] = "스크린샷 데이터를 추출할 수 없었습니다"
        ...
        "test_scenarios": [
            {
                ...
                "scenarios": [
                    ...
                    {"action": "assert", "selector": "title", "description": "페이지 제목 확인"}
                ]
            },
            {
                "name": "검색 기능 테스트", 
                ...
                "scenarios": [
                    {"action": "wait", "selector": "input[name='q']", "description": "검색 입력창 대기"},
                    {"action": "type", "selector": "input[name='q']", "value": "test", "description": "검색어 입력"},
                    {"action": "click", "selector": "input[type='submit']", "description": "검색 버튼 클릭"}
                ]
            }
        ]
    }
...
async def run_web_test(request: WebTestRequest):
    ...
    try:
        ...
        if request.custom_instruction:
            ...
            instruction = test_instructions.get(request.test_type, test_instructions["basic"])

        logger.info("/api/test/run 요청: url=%s type=%s custom=%s", request.url, request.test_type, bool(request.custom_instruction))
        ...
        try:
            result = await asyncio.wait_for(
                agent.run_web_test_natural_language(instruction), 
                timeout=30.0
            )
        except asyncio.TimeoutError:
            logger.warning("테스트 타임아웃: url=%s type=%s", request.url, request.test_type)
            raise HTTPException(
                status_code=408, 
        ...
        # AI로 테스트 요약 생성
        test_summary = await generate_test_summary(instruction, request.url, request.test_type)
        ...
        logger.info(
            ...
            result.get("test_id"), result.get("status"), result.get("execution_time", 0.0),
            result.get("events_count", 0), len(parsed_results.get("screenshots", []))
        )
        
        # AI 응답을 Q&A 형태로 구조화
        structured_qa = structure_ai_responses(parsed_results.get("model_responses", []))
        ...
        logger.exception("/api/test/run 처리 실패: url=%s type=%s", request.url, request.test_type)
        return WebTestResponse(
            test_id="error",
            status="failed",
            execution_time=0.0,
