...
class AutoTestSuiteExtension(AutoTestSuite):
    ...
    async def _generate_test_cases_from_analysis(self, page_analysis: Dict[str, Any], test_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            test_cases.append({
                ...
                "steps": [
                    ...
                    {
                        "action": "assert_title",
                        "expected_value": page_analysis.get("basic_info", {}).get("title", ""),
            ...
            # 2. 상호작용 요소 테스트
            interactive_elements = page_analysis.get("interactive_elements", {}).get("clickable_elements", [])
            ...
            if test_type in ["comprehensive", "accessibility"]:
                accessibility_tests = self._generate_accessibility_test_cases(page_analysis)
    ...
    def _generate_accessibility_test_cases(self, page_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """접근성 테스트 케이스 생성"""
        test_cases = []
        ...
        # 키보드 네비게이션 테스트
        focusable_elements = page_analysis.get("interactive_elements", {}).get("focusable_elements", [])
    ...
    def _generate_performance_test_cases(self, page_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """성능 테스트 케이스 생성"""
        test_cases = []
    ...
    async def _generate_automation_scripts(self, test_cases: List[Dict[str, Any]], page_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            python_script = self._generate_python_playwright_script(test_cases, page_analysis)
            scripts.append({
    ...
    def _generate_python_playwright_script(self, test_cases: List[Dict[str, Any]], page_analysis: Dict[str, Any]) -> str:
        """Python Playwright 스크립트 생성"""
...
'''
    ...
    def _generate_json_test_data(self, test_cases: List[Dict[str, Any]], page_analysis: Dict[str, Any]) -> str:
        """JSON 테스트 데이터 생성"""
        test_data = {
            "metadata": {
    ...
    async def _execute_generated_tests(self, test_cases: List[Dict[str, Any]], url: str) -> Dict[str, Any]:
        """생성된 테스트 케이스 실행"""
        try:
    ...
    async def _execute_test_case_with_mcp(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_step_with_mcp(self, step: Dict[str, Any]):
        ...
        if action == "navigate":
            ...
            expected_title = step.get("expected_value", "")
            actual_title = await self.mcp_client.execute_javascript("() => document.title")
            if actual_title != expected_title:
                raise Exception(f"제목 불일치: 예상={expected_title}, 실제={actual_title}")
            ...
            if images_without_alt:
                logger.warning(f"Alt 텍스트가 없는 이미지 {len(images_without_alt)}개 발견")
    ...
    async def _generate_comprehensive_report(self, page_analysis: Dict[str, Any], test_cases: List[Dict[str, Any]], 
                                           automation_scripts: List[Dict[str, Any]], execution_results: Dict[str, Any],
        ...
        try:
            ...
            report = {
                ...
                "page_analysis_summary": {
                    ...
                    "interactive_elements": len(page_analysis.get("interactive_elements", {}).get("clickable_elements", [])),
                    "forms": len(page_analysis.get("form_elements", [])),
                    "images": len(page_analysis.get("page_structure", {}).get("images", []))
                },
                "test_cases_summary": {
                    "functional_tests": len([tc for tc in test_cases if tc.get("type") == "functional"]),
                    "accessibility_tests": len([tc for tc in test_cases if tc.get("type") == "accessibility"]),
                    "performance_tests": len([tc for tc in test_cases if tc.get("type") == "performance"])
                },
                "performance_summary": {
                    "load_time": monitoring_results.get("performance_metrics", {}).get("navigationTiming", {}).get("pageLoad"),
                    "memory_usage": monitoring_results.get("memory_metrics", {}).get("heapUsagePercentage"),
                    "dom_elements": monitoring_results.get("performance_metrics", {}).get("domElements")
                },
                "recommendations": self._generate_recommendations(page_analysis, execution_results, monitoring_results),
                "generated_files": [
                    {
    ...
    def _count_total_elements(self, page_analysis: Dict[str, Any]) -> int:
        ...
        total = 0
        for element_type in ["headings", "paragraphs", "images", "links", "buttons", "inputs"]:
            total += len(page_structure.get(element_type, []))
        return total
    
    def _generate_recommendations(self, page_analysis: Dict[str, Any], execution_results: Dict[str, Any], 
                                 monitoring_results: Dict[str, Any]) -> List[str]:
        """개선 권장사항 생성"""
        recommendations = []
        
        # 성공률 기반 권장사항
        success_rate = execution_results.get("passed_tests", 0) / max(execution_results.get("total_tests", 1), 1) * 100
        if success_rate < 80:
            recommendations.append("테스트 성공률이 낮습니다. 페이지 요소의 선택자를 개선하거나 대기 시간을 늘려보세요.")
        
        # 접근성 기반 권장사항
        images = page_analysis.get("page_structure", {}).get("images", [])
        images_without_alt = [img for img in images if not img.get("alt")]
        if images_without_alt:
            recommendations.append(f"Alt 텍스트가 없는 이미지가 {len(images_without_alt)}개 있습니다. 접근성을 위해 Alt 텍스트를 추가하세요.")
        ...
        if load_time and load_time > 3000:
            recommendations.append(f"페이지 로드 시간이 {load_time}ms로 느립니다. 성능 최적화를 고려하세요.")
        
        memory_usage = monitoring_results.get("memory_metrics", {}).get("heapUsagePercentage")
        if memory_usage and memory_usage > 80:
            recommendations.append(f"메모리 사용량이 {memory_usage:.1f}%로 높습니다. 메모리 누수를 확인하세요.")
        ...
        for form in forms:
            ...
            if not required_fields:
                recommendations.append("폼에 필수 필드 표시가 없습니다. 사용자 경험을 위해 필수 필드를 명확히 표시하세요.")
