...
class AutoTestRequest(BaseModel):
    """자동 테스트 요청 모델"""
    url: str
    test_type: str = "comprehensive"  # comprehensive, functional, accessibility, performance
...
class AutoTestAPI:
    ...
    def _setup_routes(self):
        ...
        async def run_auto_test(request: AutoTestRequest, background_tasks: BackgroundTasks):
            """자동 테스트 워크플로우 실행"""
            try:
            ...
            try:
                if workflow_id in self.active_workflows:
                    ...
                    if workflow_data.get("status") == "completed":
                        ...
                        raise HTTPException(status_code=400, detail="워크플로우가 아직 완료되지 않았습니다")
                else:
                    raise HTTPException(status_code=404, detail="워크플로우를 찾을 수 없습니다")
        ...
        async def generate_test_cases_only(request: AutoTestRequest):
            ...
            try:
                ...
                # 테스트 케이스 생성
                test_cases = await self.auto_suite._generate_test_cases_from_analysis(page_analysis, request.test_type)
        ...
        async def generate_scripts_only(request: AutoTestRequest):
            ...
            try:
                ...
                # 자동화 스크립트 생성
                automation_scripts = await self.auto_suite._generate_automation_scripts(test_cases, page_analysis)
        ...
        async def monitor_performance_only(request: AutoTestRequest):
            ...
            try:
                ...
                # 성능 모니터링 수행
                monitoring_results = await self.auto_suite._perform_monitoring_and_metrics(request.url)
    ...
    async def _execute_workflow_background(self, workflow_id: str, request: AutoTestRequest):
        """백그라운드에서 워크플로우 실행"""
        try:
            ...
            # 4단계: 테스트 실행
            self.active_workflows[workflow_id].update({
                "current_step": "테스트 실행 중",
                "progress": 70
            })
            execution_results = await self.auto_suite._execute_generated_tests(test_cases, request.url)
