...
def suppress_log_messages(
    substrings: list[str], logger_names: Optional[list[str]] = None
) -> None:
...
class QARadarLogger:
    ...
    def test_complete(
        self, test_id: str, status: str, execution_time: float, quality_score: float
    ):
        """테스트 완료 로그"""
        self.info(
            f"테스트 완료 - ID: {test_id}, 상태: {status}, 실행시간: {execution_time:.2f}초, 품질점수: {quality_score:.1f}"
        )
    ...
    def quality_assessment(self, category: str, score: float, details: str = ""):
        """품질 평가 로그"""
        self.info(
            f"품질 평가 - 카테고리: {category}, 점수: {score:.1f}, 상세: {details}"
        )
