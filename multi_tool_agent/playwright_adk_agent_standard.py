...
class PlaywrightADKAgentStandard:
    ...
    async def _execute_test_scenario_standard(self, scenario: Dict[str, Any], tool_map: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "wait":
                ...
                if 'assert_element' in tool_map:
                    ...
                    return {"success": False, "action": action, "description": description, "error": "assert_element tool not available"}
                
            else:
                logger.warning(f"지원하지 않는 액션: {action}")
                return {"success": False, "action": action, "description": description, "error": "Unsupported action"}
    ...
    def _calculate_quality_score_standard(self, metrics: Dict[str, Any]) -> float:
        ...
        error_count = len([log for log in logs if "error" in log.get("level", "").lower()])
        score -= error_count * 5
