...
def check_dependencies():
    ...
    # 필요한 패키지 확인
    required_packages = ["fastapi", "uvicorn", "requests", "playwright"]
...
def check_playwright_mcp():
    ...
    try:
        ...
        result = subprocess.run(
            ["npx", "@playwright/mcp", "--version"], capture_output=True, text=True
        )
        if result.returncode != 0:
...
def create_directories():
    ...
    directories = ["screenshots", "logs", "reports", "data"]
...
def run_tests():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "test_playwright_adk.py"], capture_output=True, text=True
        )
