...
class PlaywrightADKApp:
    ...
    def _setup_routes(self):
        ...
        async def run_web_test(request: WebTestRequest, background_tasks: BackgroundTasks):
            """웹 테스트 실행"""
            try:
        ...
        async def analyze_webpage_quality(request: QualityAnalysisRequest):
            ...
            try:
                ...
                if request.include_ai_analysis:
                    ai_analysis = await self.agent.perform_ai_enhanced_analysis(quality_result)
                
                # ML 권장사항 (요청된 경우)
                ml_recommendations = None
                if request.include_ml_recommendations:
                    ml_recommendations = await self.agent.generate_ml_recommendations(quality_result)
        ...
        async def test_responsive_design(url: str, viewports: List[Dict[str, int]]):
            ...
            try:
                ...
                responsive_result = await self.agent.test_responsive_design(url, viewports)
        ...
        async def monitor_performance(url: str, duration: int = 60):
            ...
            try:
                ...
                performance_result = await self.agent.monitor_web_performance(url, duration)
        ...
        async def capture_visual_evidence(url: str, elements: List[str]):
            ...
            try:
                ...
                evidence_result = await self.agent.capture_visual_evidence(url, elements)
        ...
        async def get_test_report(test_id: str):
            ...
            try:
                ...
                if test_id in self.test_status:
                    ...
                    if current_status["status"] in ["started", "running"]:
                        return {
                            ...
                            "completed_scenarios": current_status["completed_scenarios"],
                        ...
                        error_info = current_status.get("error_message", "Unknown error")
                        del self.test_status[test_id]
                        return {
                            "test_id": test_id,
                            "status": "error",
                ...
                raise HTTPException(status_code=404, detail="테스트 리포트를 찾을 수 없습니다")
        ...
        async def get_system_status():
            ...
            try:
                return {
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "agent_status": "active",
                    "mcp_client_status": "connected" if self.agent.mcp_client.connected else "disconnected",
    ...
    async def _execute_web_test(self, test_id: str, request: WebTestRequest):
        ...
        try:
            ...
            # 1. 기본 웹 테스트 실행
            test_result = await self.agent.run_web_test(request.url, request.test_scenarios)
            ...
            if request.performance_monitoring:
                if test_id in self.test_status:
                    self.test_status[test_id].update({
                        "current_step": "성능 모니터링 중",
                        "progress": 50
                    })
                performance_result = await self.agent.monitor_web_performance(request.url, 60)
            ...
            if request.accessibility_testing:
                if test_id in self.test_status:
                    self.test_status[test_id].update({
                        "current_step": "접근성 테스트 중",
                        "progress": 60
                    })
                accessibility_result = await self.agent.analyze_accessibility(request.url)
            ...
            if request.responsive_testing:
                ...
                responsive_result = await self.agent.test_responsive_design(request.url, default_viewports)
    ...
    def _combine_recommendations(self, *results) -> List[str]:
        ...
        for result in results:
            if result and isinstance(result, dict):
                ...
                # 접근성 결과에서 권장사항
                if "accessibility_score" in result and result.get("accessibility_score", 100) < 80:
